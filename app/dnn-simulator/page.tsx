...
export default function DNNSimulator() {
  ...
  const [activeTab, setActiveTab] = useState<"charts" | "logs" | "analysis">(
    "charts",
  );
  ...
  const [activation, setActivation] =
    useState<DenseLayer["activation"]>("relu");
  ...
  useEffect(() => {
    ...
    layers.forEach((layer, idx) => {
      drawNodesForLayer(
        ...
        layer.units,
        `${layer.activation}\n(${layer.units})`,
        activationColors[layer.activation],
      );
      x += layerSpacing;
    });
    ...
    if (layers.length > 0) {
      for (let i = 0; i < Math.min(inputFeatures, 4); i++) {
        for (let j = 0; j < Math.min(layers[0].units, 4); j++) {
          ...
          ctx.lineTo(
            startX + layerSpacing,
            centerY - (layers[0].units * 30) / 2 + j * 30,
          );
          ctx.stroke();
        }
      }
  ...
  const addLayer = () => {
    ...
    addLog(
      `Added Dense layer with ${layerUnits} units (${activation} activation)`,
      "success",
    );
  };
  ...
  const buildModel = () => {
    ...
    const totalParams =
      ...
      layers[layers.length - 1].units * numClasses +
      numClasses;
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        addLog(
          `Loaded dataset with ${data.numSamples} samples and ${data.numFeatures} features`,
          "success",
        );
      } catch (err) {
  ...
  const startTraining = () => {
    ...
    addLog(
      `Using ${optimizerType.toUpperCase()} optimizer with LR=${learningRate}`,
      "info",
    );
    ...
    const interval = setInterval(
      () => {
        ...
        const newAccuracy = Math.min(
          0.99,
          0.4 + (currentEpoch / maxEpochs) * 0.55 + noise,
        );
        const newValAccuracy = Math.min(
          0.96,
          0.35 + (currentEpoch / maxEpochs) * 0.58 + noise - 0.02,
        );
        const newLoss = Math.max(
          0.05,
          3 - (currentEpoch / maxEpochs) * 2.8 + noise,
        );
  ...
  return (
    <div
      className={`min-h-screen transition-all duration-300 ${isDarkMode ? "dark-mode bg-slate-900" : "bg-white"}`}
      style={{
    ...
    >
      <div className="container mx-auto px-4 py-6">
        {/* Header */}
        <div
          className={`rounded-3xl shadow-xl overflow-hidden mb-6 transition-all duration-300 ${isDarkMode ? "bg-slate-900" : "bg-white"}`}
        >
            ...
            <div className="flex justify-between items-center flex-wrap gap-4">
              <div>
                ...
                <p className="text-white/90 text-sm">
                  Deep Neural Network Builder with Real-time Training Analysis
                </p>
              </div>
          ...
          {/* Alerts */}
          <div
            className={`px-8 py-4 ${isDarkMode ? "bg-slate-800" : "bg-slate-100"}`}
          >
            {alerts.length === 0 ? (
              <p
                className={`text-sm ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}
              >
                Build, train, and analyze deep neural networks with interactive
                visualization
              </p>
            ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            {/* Network Configuration */}
            <div
              className={`rounded-2xl p-6 transition-all duration-300 hover:shadow-xl ${isDarkMode ? "bg-slate-800 hover:border-violet-500" : "bg-white hover:border-violet-500"} border-2 border-transparent`}
            >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    value={activation}
                    onChange={(e) =>
                      setActivation(e.target.value as DenseLayer["activation"])
                    }
                    className={`w-full px-4 py-3 rounded-lg border-2 text-sm transition-all duration-300 ${
                      isDarkMode
                        ? "bg-slate-700 border-slate-600 text-white focus:border-violet-500"
            ...
            {/* Layer Stack */}
            <div
              className={`rounded-2xl p-6 transition-all duration-300 ${isDarkMode ? "bg-slate-800" : "bg-white"} border-2 border-transparent hover:border-violet-500`}
            >
              <h2 className="text-xl font-bold text-violet-600 mb-6 flex items-center gap-2">
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        >
                          ...
                          {(layer.dropout! * 100).toFixed(0)}% dropout
                        </p>
                      </div>
                      <button
            ...
            >
              ...
              <div>
                <label
                  className="block px-6 py-8 rounded-lg border-2 border-dashed transition-all duration-300 cursor-pointer text-center"
                  style={{
                ...
                >
                  ...
                  />
                  <p
                    className={`font-semibold mb-1 ${isDarkMode ? "text-violet-200" : "text-violet-600"}`}
                  >
                    Click to upload dataset
                  </p>
                  <p
                    className={`text-xs ${isDarkMode ? "text-violet-300/70" : "text-violet-600/70"}`}
                  >
                ...
                {dataLoaded && (
                  <p
                    className={`text-xs mt-3 p-2 rounded ${isDarkMode ? "bg-green-500/20 text-green-300" : "bg-green-100 text-green-700"}`}
                  >
                    ✅ Dataset loaded: {dataset?.numSamples} samples,{" "}
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {[
                  ...
                  {
                    label: "Val Acc",
                    value: `${(valAccuracy * 100).toFixed(1)}%`,
                    fill: valAccuracy * 100,
                  },
                  {
                    label: "Loss",
                    value: loss.toFixed(3),
                    fill: Math.min((1 - loss / 3) * 100, 100),
                  },
                  {
                    label: "Epochs",
                    value: `${epoch}/${maxEpochs}`,
                    fill: (epoch / maxEpochs) * 100,
                  },
                ].map((metric, idx) => (
                  <div
                    key={idx}
                  ...
                  >
                    <p
                      className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? "text-slate-400" : "text-slate-500"} mb-1`}
                    >
                      {metric.label}
                    </p>
                    ...
                    <div
                      className={`h-1.5 rounded-full overflow-hidden ${isDarkMode ? "bg-slate-600" : "bg-slate-200"}`}
                    >
                      <div
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                ...
                >
                  <p
                    className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? "text-slate-400" : "text-slate-500"} mb-2`}
                  >
                    Batch Size
                  </p>
                  <input
            ...
            >
              ...
              {activeTab === "charts" && (
                <div className="text-center py-8 text-slate-500">
                  <p>
                    Training curves and performance charts would display here
              ...
              {activeTab === "analysis" && (
                <div
                  className={`p-6 rounded-lg ${isDarkMode ? "bg-slate-700" : "bg-slate-100"}`}
                >
                  <div className="space-y-4">
                    <div>
                      <p
                        className={`text-sm font-bold mb-3 ${isDarkMode ? "text-slate-200" : "text-slate-700"}`}
                      >
                        Layer Activations
                      </p>
                          ...
                          {layerActivations.map((act, idx) => (
                            <div key={idx} className="space-y-1">
                              <div className="flex items-center justify-between">
                                ...
                                <span
                                  className={`text-xs font-mono ${isDarkMode ? "text-violet-400" : "text-violet-600"}`}
                                >
                                  {act.toFixed(1)}%
                                </span>
                              </div>
                              <div
                                className={`h-2 rounded-full overflow-hidden ${isDarkMode ? "bg-slate-600" : "bg-slate-300"}`}
                              >
                                <div
